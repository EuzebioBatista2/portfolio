...
export default function CardComment(props: ICardCommentProps) {
    return (
        <div className={`flex flex-col w-3/4 sm:w-1/3 p-2 border border-gray-800 dark:border-black rounded-md my-4 bg-gray-100 dark:bg-gray-700
        ${props.side === 'start' ? 'self-start animate-bounce-slow' : 'self-end animate-bounce-verySlow' }
        `}>
            <div className="flex items-center justify-center mb-2">
                <div className={`flex justify-center border h-7 w-7 rounded-full overflow-hidden mr-1 ${props.side === 'start' ? 'border-blue-600' : 'border-orange-400' }`}>
                    <i className={`flex h-8 w-8 ${props.side === 'start' ? 'fill-blue-500' : 'fill-orange-400' }`}>{IconUserB}</i>
                </div>
